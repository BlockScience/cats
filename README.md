...
- **Summarized** *Loop* below:
  - **A: Manifactured Execution** - CATs (*CAT workloads*) are **Ordered** and **Invoiced** for verification and registered as **[BOMs](docs/provenance/BOM.md)** to serve as **Data Provenance records** that uniquely identify CATs and their content for **verifiable data processing** using content-addresses. CATs are deployable as parallelized and distributed processes to support scalable data processing microservices.
  ...
  - **B: Content-Addressed Bills-of-Materials (BOMs)** - *BOMs* employ Node **CAS-over-HTTP** for [Data Verification](https://en.wikipedia.org/wiki/Data_verification) and **content-address retrieval over HTTP URI** (`*_uri` locators; equality as `ni:`). That uniquely identifies CAT content and retrieves it without IPFS CIDs on the live path. CATs' use of this content-addressing mechanism establishes a self-service Data Mesh as a heterogeneously scalable Compute Platform deployable on [Kubernetes](https://kubernetes.io/) as CAT Node's execution paradigm (Structure [PaaS]).
    - **3.** the *Node's* **Runtime** *emits* a **BOM** as the *Mesh-transportable Data Provenance record* to be *shared* & *re-executed* between Node; a Node-local **BOM registry** indexes verified envelopes so the next Order can be discovered via `content_id` / `data_uri` / `bom_ldp_uri` (not only out-of-band `order_uri`) — see [BomRegistry.md](./docs/provenance/BomRegistry.md) / [ControlFeedbackLoop.md](./docs/architecture/ControlFeedbackLoop.md)
...
`make deps-all` - Runs on macOS or Linux (see the [Makefile](./Makefile) and `make help`), or follow [DEPS.md](./docs/guide/DEPS.md) to install each dependency manually.

#### 2. [Storage](./docs/storage/STORAGE.md) — content-store (CAS + optional Kubo):
...
Does **not** start Kubo — that is `content-store-ensure` / `node-up`. Details: [STORAGE.md](./docs/storage/STORAGE.md) / [IPFS.md](./docs/storage/IPFS.md).

#### 3. [Node Lifecycle](./docs/guide/NodeLifeCycle.md) — start / status / stop:
...
Full command reference: [NodeLifeCycle.md](./docs/guide/NodeLifeCycle.md).

#### 4. [Demonstration](./docs/guide/DEMO.md):
...
#### 5. [Testing:](./docs/guide/TEST.md) *CAT Node Integration & Unit Tests*

#### 6. [Dashboards](./docs/guide/DASHBOARDS.md)
...
- **[Content-Addressing](docs/provenance/CAS.md)** - reffers to **Content-Addressed Storage (CAS)**: uniquely identify by content (`ni:`) and retrieve over HTTP URI (`*_uri` / Node CAS-over-HTTP), rather than by a location-only address or an IPFS CID.
