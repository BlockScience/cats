...
def _(mo):
    ...
    This notebook is [`cats_lineage_demo.py`](cats_lineage_demo.py) (registry-first CAT0 → CAT1). Mesh establish remains [`cats_demo.py`](cats_demo.py).
    ...
    **Provenance** is the **record** (signed BOM / Invoice / Order binding — what ran, with what). **Lineage** is the **path** (how data or Process/Function connect — intra-run hops or CAT→CAT equality). This notebook demonstrates **data and process** of both:
    ...
    Not demonstrated here: `linkStructure` (Structure mutation); mesh-federated registry; dataframe transform checks (`tests/test_provenance.py`).
    ...
    * **CAT1** (*compose the next Data Product from the catalog, then prove lineage and this run’s provenance* — **process lineage**, **cross-CAT data lineage**, then a **new record** on CAT1’s own envelope). Stewardship/composition uses `linkProcess(content_id=cat0_data_id)` (or `bom_ldp_uri=` if `by-data` is ambiguous), hotF `process_1`. Lineage first (`flatten_bom` + pairing / data-chain / distinct execution). Then the same CAT0 record helpers on **this** BOM (`bom.invoice_uri` / LDP) — not CAT0’s inspect tree. That is lineage of provenance on the mesh without embedding a pointer to CAT0’s BOM in CAT1’s bytes.
    """)
    return
...
def _(mo):
    ...
    - **CAT1 — process lineage + cross-CAT data lineage, then this run’s provenance:** `assert_order_pairing_lineage`, `assert_invoice_data_chain`, `assert_distinct_executions`; then handoff / `assert_*_content_equiv` / `assert_execution_bind` / `stageLineage` / as-executed on CAT1’s envelope
    """)
    return
...
def cat0_inspection(mo):
    ...
    **Control-Feedback Loop** (this section’s purpose: *mint and prove one run*):
    ...
    Mesh client and registry (`mesh_client`) and inspect helpers (`inspect_helpers`) are bound **above**. Inspect helpers stay off the mint path so editing asserts does not remint. CAS-only Node client (`ContentMesh(ipfsClient=None)`). Node up (`make node-start` / `make node-up`). See [`DEMO.md`](../docs/guide/DEMO.md). Same I/O as [`cats_demo.py`](cats_demo.py); CAT0 does **not** call `flatten_bom`.
...
def cat1_inspection(mo):
    ...
    **Control-Feedback Loop** (this section’s purpose: *compose from the index, then prove lineage*):
    ...
    4. **Feedback inspect (`flatten_bom` tree)** — `{invoice, log}` with uri slots + parent `flat` (`cat1_flatten_bom`). **Process lineage:** Function mutated, Structure carried (`assert_order_pairing_lineage`). **Cross-CAT data lineage:** CAT1 **input** Invoice data ≡ CAT0 **output** data (`assert_invoice_data_chain`: `order.flat.invoice`, not the Executor **output** Invoice). **New execution:** output data and seeds differ (`assert_distinct_executions`).
    5. **This run’s provenance (new record)** — same CAT0 helpers on CAT1’s envelope (`cat1_bom`, `cat1_invoice`, `cat1_order`, `cat1_execution_bind`, `cat1_stage_*`). Not a replay of CAT0’s locator tree.

    `mesh_client` / `inspect_helpers` are bound **above**. `tests/test_lineage.py` covers the three lineage helpers; `tests/test_provenance.py` CAT1 is a second `create_order_request(process_1)` (not `linkProcess` data-chain).
    ...
    **Acceptance criteria** (proven by the cells below):
    ...
    | CAT1 signed BOM → Invoice → Order cohere; mesh.cat ≡ HTTP (**this run’s data provenance**) | `cat1_bom` |
    | CAT1 Invoice `data` / stages / SAE + seed; Order Function / Structure slots (**this run’s record**) | `cat1_invoice`, `cat1_order` |
    | `prov:used` Order then Invoice; `stageLineage` hops; as-executed bind (**this run**, not CAT0) | `cat1_execution_bind`, `cat1_stage_lineage_payload_equiv`, `cat1_stage3_*` |

    Not in this section: a second `create_order_request`; `linkStructure`; mesh-federated registry; dataframe transform checks (`tests/test_provenance.py`). CAT0’s locator-only walk stays in the CAT0 section.
    """)
    return
...
def _(mo):
    ...
    **No** top-level envelope `invoice_uri` / `order_uri`. Lineage inspect via
    `flatten_bom` next; this run’s provenance walk (handoff / HTTP) follows
    those lineage asserts.
    """)
    return
...
def _(mo):
    ...
    (`assert_invoice_data_chain`), and distinct-execution (`assert_distinct_executions`).
    CAT1 provenance cells **after** those lineage asserts walk **this** envelope
    (handoff / content-equiv / `stageLineage`), not CAT0’s inspect tree.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ## Inspect CAT1 BOM & nested Envelopes (this run’s provenance)

    **New record**, not a CAT0 replay. Same helpers as CAT0 on CAT1’s envelope:
    signed `bom.invoice_uri` → Invoice → Order. `cat1_record` is the registry
    row for **this** run (`cat1_registry_index_parity`). `flatten_bom` above
    stays the lineage inspect tree; these cells use HTTP / handoff like CAT0.

    **`assert_control_plane_handoff_coherence`** then **`assert_*_content_equiv`**.
    """)
    return
...
def cat1_bom(
    ...
    cat1_handoff = assert_control_plane_handoff_coherence(
        cat_response=cat1_bom_response,
        record=cat1_record,
        http_get_json=http_get_json,
        content_mesh=contentMesh,
    )
    cat1_fetch_ref = cat1_handoff["fetch_ref"]
    cat1_bom = cat1_handoff["bom"]
    assert_bom_content_equiv(
        cat1_bom,
        content_mesh=contentMesh,
        http_get_json=http_get_json,
    ...
    cat1_bom
    return cat1_bom, cat1_fetch_ref, cat1_handoff


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""
    ## Inspect CAT1 Invoice Envelope

    **This run’s data provenance** (`data` / `data_stages` / as-executed) and
    **process provenance** (`seed`). Then `flatten_uri_dict` (assert and flatten stay separate).
    """)
    return
...
def cat1_invoice(
    ...
    cat1_raw_invoice = cat1_handoff["invoice"]
    assert_invoice_content_equiv(
        cat1_raw_invoice,
        content_mesh=contentMesh,
        http_get_json=http_get_json,
    ...
    cat1_flat_invoice = flatten_uri_dict(
        cat1_raw_invoice, cat1_fetch_ref, max_depth=2, stems=_invoice_stems_1
    )
    print("Invoice (CAT1):")
    pprint(cat1_raw_invoice)
    cat1_flat_invoice
    return
...
def _(mo):
    mo.md(r"""
    ### Inspect CAT1 Order Envelope

    **This run’s process provenance** (Function / Structure as-Code after
    `linkProcess`). Nested input Invoice is CAT0’s output lot (**input data
    provenance**), already chained above.
    """)
    return
...
def cat1_order(
    ...
    cat1_raw_order = cat1_handoff["order"]
    assert_order_content_equiv(
        cat1_raw_order,
        content_mesh=contentMesh,
        http_get_json=http_get_json,
    ...
    cat1_raw_function = requests.get(
        cat1_raw_order["function_uri"], timeout=60
    ).json()
    assert_order_function_slots(cat1_raw_function)
    assert_order_function_content_equiv(
        cat1_raw_function,
        content_mesh=contentMesh,
        http_get_json=http_get_json,
    ...
    cat1_raw_structure = requests.get(
        cat1_raw_order["structure_uri"], timeout=60
    ).json()
    assert_order_structure_slots(cat1_raw_structure)
    assert_order_structure_content_equiv(
        cat1_raw_structure,
        content_mesh=contentMesh,
        http_get_json=http_get_json,
    ...
    cat1_raw_ordered_invoice = requests.get(
        cat1_raw_order["invoice_uri"], timeout=60
    ).json()
    assert_input_invoice_slots(cat1_raw_ordered_invoice)
    assert_order_invoice_content_equiv(
        cat1_raw_ordered_invoice,
        content_mesh=contentMesh,
        http_get_json=http_get_json,
        http_get=http_get,
        record=cat1_record,
...
@app.cell(hide_code=True)
def cat1_order_display(
    cat1_fetch_ref,
    cat1_raw_order,
    flatten_uri_dict,
    pprint,
):
    cat1_flat_order = flatten_uri_dict(
        cat1_raw_order,
        cat1_fetch_ref,
        max_depth=1,
        stems={"function", "structure", "invoice"},
    ...
    print("Order (CAT1):")
    pprint(cat1_raw_order)
    cat1_flat_order
    return


@app.cell(hide_code=True)
def cat1_function_display(
    cat1_fetch_ref,
    cat1_raw_function,
    flatten_uri_dict,
    pprint,
):
    cat1_flat_function = flatten_uri_dict(
        cat1_raw_function,
        cat1_fetch_ref,
        max_depth=1,
        stems={
    ...
    print("Function (CAT1):")
    pprint(cat1_raw_function)
    cat1_flat_function
    return


@app.cell(hide_code=True)
def cat1_structure_display(
    cat1_fetch_ref,
    cat1_raw_structure,
    flatten_uri_dict,
    pprint,
):
    cat1_flat_structure = flatten_uri_dict(
        cat1_raw_structure,
        cat1_fetch_ref,
        max_depth=1,
        stems={"infrastructure", "plant", "root"},
    )
    print("Structure (CAT1):")
    pprint(cat1_raw_structure)
    cat1_flat_structure
    return


@app.cell
def cat1_ordered_invoice_display(
    cat1_raw_ordered_invoice,
    flatten_uri_dict,
    http_get_json,
):
    cat1_flat_ordered_invoice = flatten_uri_dict(
        cat1_raw_ordered_invoice,
        lambda stem, uri: http_get_json(uri),
        max_depth=1,
        stems={"data"},
    )
    print("Ordered Invoice (CAT1):")
    print(cat1_raw_ordered_invoice)
    cat1_flat_ordered_invoice
    return
...
def _(mo):
    mo.md(r"""
    ### Inspect CAT1 Execution Provenance

    Same bind and intra-run hops as CAT0, on **this** BOM. Order is process
    provenance; Invoice is data provenance. As-executed is observation, not a
    payload hop. Not CAT0’s `stageLineage` entities.
    """)
    return
...
def _(mo):
    ...
    On **this** BOM: `prov:used` is CAT1’s Order (`process_1` + carried
    Structure) then CAT1’s output Invoice. Not CAT0’s Order / Invoice.
    """)
    return


@app.cell
def cat1_execution_bind(
    assert_execution_bind,
    cat1_bom,
    cat1_handoff,
    http_get_json,
):
    cat1_execution_bind = assert_execution_bind(
        cat1_bom,
        order_uri=cat1_handoff["invoice"]["order_uri"],
        invoice_uri=cat1_handoff["invoice_uri"],
        http_get_json=http_get_json,
        expected_order=cat1_handoff["order"],
        expected_invoice=cat1_handoff["invoice"],
    )
    cat1_execution_bind
    return (cat1_execution_bind,)


@app.cell
def cat1_generated_invoice(cat1_execution_bind, requests):
    _invoice_url_1 = cat1_execution_bind[1]["@id"]
    _invoice_resp_1 = requests.get(_invoice_url_1, timeout=60)
    _invoice_resp_1.raise_for_status()
    cat1_generated_invoice = _invoice_resp_1.json()
    cat1_generated_invoice
    return


@app.cell
def cat1_used_order(cat1_execution_bind, requests):
    _order_url_1 = cat1_execution_bind[0]["@id"]
    _order_resp_1 = requests.get(_order_url_1, timeout=60)
    _order_resp_1.raise_for_status()
    cat1_used_order = _order_resp_1.json()
    cat1_used_order
    return
...
def _(mo):
    mo.md(r"""
    ##### Stage lineage payload hops

    CAT1’s own `stageLineage`. Ingress input **is** CAT0 egress (data
    equality already asserted above). SAE (`cat1_stage3_*`) is CAT1’s apply
    — observation on this run, not a payload hop and not CAT0’s SAE.
    """)
    return


@app.cell
def cat1_stage_lineage_payload_equiv(
    assert_stage_lineage_payload_equiv,
    cat1_bom,
    http_get_json,
):
    assert_stage_lineage_payload_equiv(cat1_bom, http_get_json=http_get_json)
    return


@app.cell
def cat1_stage0_ingress(
    assert_directory_manifest_equiv,
    cat1_bom,
    http_get_json,
    requests,
):
    cat1_ingress_stage = cat1_bom["stageLineage"][0]
    cat1_ingress_uri = cat1_ingress_stage["@id"]
    _cat1_input_data_uri = cat1_ingress_stage["prov:wasDerivedFrom"]["@id"]
    _ingressed_resp_1 = requests.get(cat1_ingress_uri, timeout=60)
    _ingressed_resp_1.raise_for_status()
    cat1_ingressed_data = _ingressed_resp_1.json()
    assert_directory_manifest_equiv(
        cat1_ingress_uri, _cat1_input_data_uri, http_get_json=http_get_json
    )
    print("CAT1 ingress_data wasDerivedFrom input_data: ok")
    cat1_ingressed_data
    return (cat1_ingress_uri,)


@app.cell
def cat1_stage1_integration(
    assert_directory_manifest_equiv,
    cat1_bom,
    cat1_ingress_uri,
    http_get_json,
    ...
    cat1_integration_stage = cat1_bom["stageLineage"][1]
    cat1_integration_uri = cat1_integration_stage["@id"]
    _cat1_integration_from = cat1_integration_stage["prov:wasDerivedFrom"]["@id"]
    _integration_resp_1 = requests.get(cat1_integration_uri, timeout=60)
    _integration_resp_1.raise_for_status()
    cat1_integration_data = _integration_resp_1.json()
    assert_directory_manifest_equiv(
        _cat1_integration_from,
        cat1_ingress_uri,
        http_get_json=http_get_json,
    )
    print("CAT1 integration_data wasDerivedFrom ingress_data: ok")
    cat1_integration_data
    return (cat1_integration_uri,)


@app.cell
def cat1_stage2_egress(
    assert_directory_manifest_equiv,
    cat1_bom,
    cat1_integration_uri,
    http_get_json,
    requests,
):
    cat1_egressed_stage = cat1_bom["stageLineage"][2]
    _cat1_egressed_uri = cat1_egressed_stage["@id"]
    _cat1_egressed_from = cat1_egressed_stage["prov:wasDerivedFrom"]["@id"]
    _egressed_resp_1 = requests.get(_cat1_egressed_uri, timeout=60)
    _egressed_resp_1.raise_for_status()
    cat1_egressed_data = _egressed_resp_1.json()
    assert_directory_manifest_equiv(
        _cat1_egressed_from,
        cat1_integration_uri,
        http_get_json=http_get_json,
    )
    print("CAT1 data wasDerivedFrom integration_data: ok")
    cat1_egressed_data
    return


@app.cell
def cat1_stage3_structure_as_executed(
    assert_structure_as_executed_bind,
    assert_structure_as_executed_slots,
    cat1_bom,
    cat1_handoff,
    ...
    _sae_uri_1 = cat1_handoff["invoice"]["structure_as_executed_uri"]
    _sae_entity_1 = assert_structure_as_executed_bind(
        cat1_bom,
        structure_as_executed_uri=_sae_uri_1,
        http_get_json=http_get_json,
        expected_structure_as_executed=http_get_json(_sae_uri_1),
    )
    _sae_resp_1 = requests.get(_sae_entity_1["@id"], timeout=60)
    _sae_resp_1.raise_for_status()
    cat1_structure_as_executed = _sae_resp_1.json()
    assert_structure_as_executed_slots(cat1_structure_as_executed)
    cat1_structure_as_executed
    return (cat1_structure_as_executed,)


@app.cell
def cat1_stage3_plant_as_executed(
    assert_plant_as_executed_snapshot,
    cat1_handoff,
    cat1_structure_as_executed,
    ref_id,
    ...
    _plant_uri_1 = cat1_structure_as_executed["plant_as_executed_uri"]
    _plant_resp_1 = requests.get(_plant_uri_1, timeout=60)
    _plant_resp_1.raise_for_status()
    cat1_plant_as_executed = _plant_resp_1.json()
    assert_plant_as_executed_snapshot(
        cat1_plant_as_executed,
        structure_id=ref_id(cat1_handoff["order"], "structure"),
    )
    cat1_plant_as_executed
    return
...
@app.cell
def cat1_stage3_infrastructure_as_executed(
    assert_infrastructure_as_executed_slots,
    cat1_structure_as_executed,
    pprint,
    requests,
):
    _infra_uri_1 = cat1_structure_as_executed["infrastructure_as_executed_uri"]
    _infra_resp_1 = requests.get(_infra_uri_1, timeout=60)
    _infra_resp_1.raise_for_status()
    cat1_infrastructure_as_executed = _infra_resp_1.json()
    assert_infrastructure_as_executed_slots(cat1_infrastructure_as_executed)
    _obj_uri_1 = cat1_infrastructure_as_executed["object_store_as_executed_uri"]
    _obj_resp_1 = requests.get(_obj_uri_1, timeout=60)
    _obj_resp_1.raise_for_status()
    cat1_object_store_as_executed = _obj_resp_1.json()
    pprint(cat1_infrastructure_as_executed)
    cat1_object_store_as_executed
    return
