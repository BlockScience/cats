...
- **Retrieval** is **CAS-over-HTTP**: HTTP **`*_uri`** locators (`GET /ldp/cas/<hex>`, Invoice/Order/BOM LDP URIs). Mesh `cat` / AddressStore resolve `ni:` / `hl:` / `http(s)` and sha256-verify. See [`STORAGE.md`](../storage/STORAGE.md) / [`IPFS.md`](../storage/IPFS.md) / [`BOM.md`](BOM.md).
